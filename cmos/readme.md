...
- Moore's law is still being kept alive, except the initially monolithic notion of IC (on a single piece of silicon) has now expanded to _chiplet_ technology, where multiple dies are integrated inside a single package, often on an interposer or using advanced packaging (2.5D or 3D).
- HKMG was introduced after CMOS scaling hit the leakage limit of SiO₂/poly-Si gates, providing low leakage, high performance, and continued transistor miniaturization (for 45nm and below).
